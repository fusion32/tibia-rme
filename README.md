...
The game server should be able to read the modified spawns, houses, and marks files as normal. The big difference, and the reason why the map is stored the way it is, is that the any patches in `game/save` are consumed **at startup** and applied to both persistent (`game/map`) and baseline (`game/origmap`) maps. Once it's done, it's done, and unless you made backup of both, there is no coming back. Next time you edit the project's map, you should have the project's baseline map synced with the server's, which is why you should commit patches locally when exporting to the server.
